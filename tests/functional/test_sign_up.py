...
async def test_sign_up_access_code_success(
    sign_up_session_get_email: SignUpSessionDB, client: AsyncClient
):
    token = create_access_token(
        sign_up_session_get_email.pk, timedelta(minutes=1)
    )
    payload = {"access_code": sign_up_session_get_email.access_code}
    response = await client.post(
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["email"] == sign_up_session_get_email.email
...
async def test_sign_up_access_code_error(
    ...
    await sign_up_session_get_email.update(
...
async def test_sign_up_name(
    sign_up_session_get_access_code: SignUpSessionDB, client: AsyncClient
):
    payload = {"username": "test"}
    token = create_access_token(
        sign_up_session_get_access_code.pk, timedelta(minutes=1)
    )
    response = await client.post(
    ...
    assert response.json() == {
        "id": sign_up_session_get_access_code.pk,
        "email": sign_up_session_get_access_code.email,
        "username": "test",
    }
...
async def test_sign_up_name_error(
    payload: Dict[str, str],
    expected_status_code: int,
    sign_up_session_get_access_code: SignUpSessionDB,
    another_user: UserDB,
    client: AsyncClient,
):
    token = create_access_token(
